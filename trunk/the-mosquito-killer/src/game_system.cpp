...
const float HAND_CHECK_RIGHT_HAND_SPEED = 3.0f;
...
void GameSystem::createWeapon(WEAPON_TYPE type, const char *modelName, float mass, float *scale, float *pos, float *quat, int score, int otherData,float* tar)
{
	switch(type)
	{
	case eWeaponBook:
	...
	m_vWeapon.back()->create(modelName, mass, scale, pos, quat, score,tar);
}
...
void GameSystem::initPlayer(void)
{
	...
	EntSight = m_pSceneMgr->createEntity("Target","cube.mesh");
...
void GameSystem::updatePlayer(const NUI_SKELETON_FRAME &frame)
{
	...
	Elbow[0] = frame.SkeletonData[m_iCurrentID].SkeletonPositions[5].x;
	Elbow[1] = frame.SkeletonData[m_iCurrentID].SkeletonPositions[5].y;
	Elbow[2] = frame.SkeletonData[m_iCurrentID].SkeletonPositions[5].z;
}
	...
	case eOnHandWaitShoot:
		{
			...
			//left_hand - left_elbow = target_direction
			
			float TargetDirect[3];
			TargetDirect[0] = leftPos[0] - Elbow[0];
			TargetDirect[1] = leftPos[1] - Elbow[1];
			TargetDirect[2] = leftPos[2] - Elbow[2];

			Ogre::Vector3 vleftPos = Ogre::Vector3(leftPos[0],leftPos[1],leftPos[2]);
			Ogre::Vector3 vTargetDirect = Ogre::Vector3(TargetDirect[0],TargetDirect[1],TargetDirect[2]);

			Ogre::Ray SightRay(vleftPos, vTargetDirect*600);
			...
			{
				//tx = itr->worldFragment->singleIntersection.x;
				Ogre::SceneNode* tsn = itr->movable->getParentSceneNode();
				//ty = itr->worldFragment->singleIntersection.y;
				//tz = itr->worldFragment->singleIntersection.z;


				//Ogre::Vector3 TargetSite = Ogre::Vector3(tx,ty,tz);
				Ogre::Vector3 TargetSite = tsn->getPosition();
				Ogre::Vector3 SightSite = TargetSite - vTargetDirect ; //maybe the site is worng

				NodeSight->setPosition(SightSite);
				//NodeSight->lookAt(); //getOrientation
			}
			...
			if(m_fShootTimePass > HAND_CHECK_SHOOT_TIMEPASS)
			{
				...
				if(speed > HAND_CHECK_RIGHT_HAND_SPEED)
				{
					...
					float scale[3] = {1.0, 1.0, 1.0};
					float tar[3] = {0,50,-200};  // {tx,ty,tz};

					if(NumBomb > 0)
					{
						//createWeapon(eWeaponBomb,"bomb.mesh", 1.0, scale, pos, quat, 100,1,tar);
						NumBomb--;
					}
					else if(NumBook > 0)
					{
						//createWeapon(eWeaponBook,"bomb.mesh", 1.0, scale, pos, quat, 100,1,tar);
						NumBook--;
					}
					else
					{
						//createWeapon(eWeaponKnife,"bomb.mesh", 1.0, scale, pos, quat, 100,1,tar);
						PhysicRigidBody *body = createRidigBody("bomb.mesh", 1.0, scale, pos, quat, NULL, ScoreSystem::createScoreObject(SCORE_TYPE_WEAPON), 8);
						body->force(TargetDirect[0], TargetDirect[1], TargetDirect[2], 0.0, 0.0, 0.0, 35);
					}
				}
